...
def line_cnn_sliding_window(
    ...
    conved_convnet_outputs = Conv2D(num_classes, (width, 128), (width, 1),
                                    activation='softmax')(convnet_outputs_extra_dim)
    
    squeezed_outputs = Lambda(lambda x: tf.squeeze(x,2))(conved_convnet_outputs)
    
    softmax_output = Lambda(lambda x: x[:, :output_length, :])(squeezed_outputs)
